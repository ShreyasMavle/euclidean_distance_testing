...
if __name__ == '__main__':
    ...
    for i in range(input_processes + 1):
        process = multiprocessing.Process(target=connect, args=(i, ))
        processes.append(process)
        process.start()

    x = [x.join() for x in processes]
    finish_time = time.time()

    print(
        f"Finished in {round((finish_time - start_time), 2)} second(s) using Multiprocessing")
    print(
        f'Average time for each request = {round((finish_time - start_time), 2) / input_processes} second(s)')
